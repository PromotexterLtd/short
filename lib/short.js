...
exports.generate = function(document, numberOfRetries) {
  ...
  if (document.hasOwnProperty('hash')) {
    ...
    console.log('generatePromise.....',generatePromise);


    generatePromise.then(function(ShortURLObject) {
        console.log('ShortURLObject......', ShortURLObject);
        console.log('ShortURLObject......', promise.resolve(ShortURLObject));
    promise.resolve(ShortURLObject);
  }, function(error) {
        console.log('error', error);
      // promise.resolve(exports.generate(document));
      console.log('Angie.....here. errror');
      if(numberOfRetries > maxRetry){
           promise.reject(error, true);

      }else{
          exports.generate(document, numberOfRetries+1);
      }
